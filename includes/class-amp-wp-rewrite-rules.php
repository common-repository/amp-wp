...
// Amp_WP_Rewrite_Rules.
$GLOBALS['amp_wp_rewrite_rules'] = new Amp_WP_Rewrite_Rules();
$GLOBALS['amp_wp_rewrite_rules']->init();

/**
 * Add a Start Point to Rewrite Rules
 *
 * @global Amp_WP_Rewrite_Rules    $amp_wp_rewrite_rules AMP WP Rewrite API
 * @param string $name
 * @param int    $places
 *
 * @since   1.0.0
 */
function amp_wp_add_rewrite_start_point( $name, $places ) {
	global $amp_wp_rewrite_rules;
	$amp_wp_rewrite_rules->add_start_point( $name, $places );
}
 ...
 * @since   1.4.0
 */
function amp_wp_add_rewrite_end_point( $name, $places ) {
	global $amp_wp_rewrite_rules;
	$amp_wp_rewrite_rules->add_end_point( $name, $places );
}
